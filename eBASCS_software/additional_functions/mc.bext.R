...
mc.bext <- function(fix_runs,online_ordering,rjmcmc_run,w,allocate_curr,mu_curr,eparas_all,ewt_all,k_curr,mix_num){
    ...
    for (i in 1:k_curr){      
      ...
      if (count_vector[i]>0){
        ...
        if (rjmcmc_run < adapt_end){
          ...
          #mu_prop[,i] <- abs(mu_prop[,i])
          #if (mu_prop[1,i] > xup) {mu_prop[1,i] <- 2*xup - mu_prop[1,i]}
          #if (mu_prop[2,i] > yup) {mu_prop[2,i] <- 2*yup - mu_prop[2,i]}
        }
  ...
  value <- list(c(k_curr,c(mu_curr),c(w)),allocate_curr, eparas_all, ewt_all)
  return(value)
}